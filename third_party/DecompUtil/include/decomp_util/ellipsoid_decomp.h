...
template <int Dim=2>
class EllipsoidDecomp2D: public EllipsoidDecomp<Dim>{
  public:
    EllipsoidDecomp2D() {} 
    EllipsoidDecomp2D(const Vecf<Dim> &origin, const Vecf<Dim> &dim) : EllipsoidDecomp<Dim>(origin, dim) {}
    ...
    */
    void dilate(const vec_Vecf<Dim> &path, double offset_x = 0) {
      ...
      this->lines_.resize(N);
      this->ellipsoids_.resize(N);
      this->polyhedrons_.resize(N);

      for (unsigned int i = 0; i < N; i++) {
        this->lines_[i] = std::make_shared<LineSegment2D<Dim>>(path[i], path[i+1]);
        this->lines_[i]->set_local_bbox(this->local_bbox_);
        this->lines_[i]->set_obs(this->obs_);
        this->lines_[i]->dilate(offset_x);

        this->ellipsoids_[i] = this->lines_[i]->get_ellipsoid();
        this->polyhedrons_[i] = this->lines_[i]->get_polyhedron();
      }


      this->path_ = path;

      if(this->global_bbox_min_.norm() != 0 || this->global_bbox_max_.norm() != 0) {
        for(auto& it: this->polyhedrons_)
          add_global_bbox(it);
      }
    }

    void add_global_bbox(Polyhedron<Dim> &Vs) {
      //**** add bound along X, Y axis

      //*** X
      Vs.add(Hyperplane2D(Vec2f(this->global_bbox_max_(0), 0), Vec2f(1, 0)));
      Vs.add(Hyperplane2D(Vec2f(this->global_bbox_min_(0), 0), Vec2f(-1, 0)));
      //*** Y
      Vs.add(Hyperplane2D(Vec2f(0, this->global_bbox_max_(1)), Vec2f(0, 1)));
      Vs.add(Hyperplane2D(Vec2f(0, this->global_bbox_min_(1)), Vec2f(0, -1)));
    }
};

template <int Dim=3>
class EllipsoidDecomp3D: public EllipsoidDecomp<Dim>{
  public:
    EllipsoidDecomp3D() {}
    EllipsoidDecomp3D(const Vecf<Dim> &origin, const Vecf<Dim> &dim) : EllipsoidDecomp<Dim>(origin, dim) {}
    /**
    * @brief Decomposition thread
    * @param path The path to dilate
    * @param offset_x offset added to the long semi-axis, default is 0
    */
    void dilate(const vec_Vecf<Dim> &path, double offset_x = 0) {
      ...
      for (unsigned int i = 0; i < N; i++) {
        this->lines_[i] = std::make_shared<LineSegment3D<Dim>>(path[i], path[i+1]);
        this->lines_[i]->set_local_bbox(this->local_bbox_);
        this->lines_[i]->set_obs(this->obs_);
        this->lines_[i]->dilate(offset_x);
  ...
  void add_global_bbox(Polyhedron<Dim> &Vs) {
     ...
     //*** Z
     Vs.add(Hyperplane3D(Vec3f(0, 0, this->global_bbox_max_(2)), Vec3f(0, 0, 1)));
     Vs.add(Hyperplane3D(Vec3f(0, 0, this->global_bbox_min_(2)), Vec3f(0, 0, -1)));

     //*** X
     Vs.add(Hyperplane3D(Vec3f(this->global_bbox_max_(0), 0, 0), Vec3f(1, 0, 0)));
     Vs.add(Hyperplane3D(Vec3f(this->global_bbox_min_(0), 0, 0), Vec3f(-1, 0, 0)));
     //*** Y
     Vs.add(Hyperplane3D(Vec3f(0, this->global_bbox_max_(1), 0), Vec3f(0, 1, 0)));
     Vs.add(Hyperplane3D(Vec3f(0, this->global_bbox_max_(1), 0), Vec3f(0, -1, 0)));
   }
};
