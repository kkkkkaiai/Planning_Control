...
def spline_opti():
    ...
    velocity_term = 0
    for i in range(len(x)-2):
        velocity_term += (x_opt[:, i+2] - 2*x_opt[:, i+1] + x_opt[:, i]) * (x_opt[:, i+2] - 2*x_opt[:, i+1] + x_opt[:, i])

    # objective function(the term must be scalar)
    opti.minimize(ca.sum1(smooth_term) + 2*ca.sum1(guidance_term) + 0.2*ca.sum1(obs_term) + 0.1*ca.sum1(velocity_term))
    ...
    # solve the problem
    opti.solver('ipopt', {'expand': False, 'ipopt.max_iter': 10000, 'ipopt.print_level': 0, 'print_time':1, 'verbose': False})
    sol = opti.solve()
