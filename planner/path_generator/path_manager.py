...
class PathManager:
    ...
    def filter_path(self, step_size=0.5):
        # filter path, if the distance between two points is less than step_size, then remove the point
        filter_path_index = []
        for i in range(0, len(self._path)-1):
            filter_path_index.append(i)
            for j in range(i+1, len(self._path)-1):
                if np.linalg.norm(self._path[j]-self._path[i]) > step_size/self._resolution:
                    i = j
                    break
        filter_path_index.append(-1)

        return self._path[filter_path_index]
    ...
    def spline_interpolate(self, path=None, ds=0.2, target_speed=1.0):
        if path is None:
            path = self._path
        cx, cy, cyaw, ck, s = calc_spline_course(path[:, 0]*self._resolution, path[:, 1]*self._resolution, ds=ds)
        self._sp = calc_speed_profile(cx, cy, cyaw, target_speed)
        ...
        try:
            if self._ref_path is not None:
                ...
                plot_path(np.array([np.array(self._ref_path.cx)/self._resolution, np.array(self._ref_path.cy)/self._resolution]).T, \
                          color='cyan', linestyle='dot', label='interpolated path')
        except:
